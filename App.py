...
import io
import os
import tempfile  # <-- Added to handle file decoding safely
...
if uploaded_file is not None:
    ...
    with st.spinner("⚡ DECRYPTING & PARSING SIGNAL MATRIX (RAM-OPTIMIZED)..."):
        try:
            ...
            # Create a temporary file on disk so Librosa's backend can decode all formats safely
            with tempfile.NamedTemporaryFile(delete=False, suffix=file_suffix) as temp_audio:
                temp_audio.write(file_bytes)
                temp_path = temp_audio.name
            
            try:
                # Load from temp file instead of BytesIO
                y, sr = librosa.load(temp_path, sr=16000, duration=20.0)
            finally:
                # Clean up file from the server immediately after load
                if os.path.exists(temp_path):
                    os.remove(temp_path)
            
            # If user uploaded a massive audio file, give them a warning
            if len(file_bytes) > 5 * 1024 * 1024:
                st.warning("⚠️ Large file detected. Parsing first 20 seconds of audio to preserve server stability.")
                
        except Exception as decrypt_err:
            st.error(f"SYSTEM ALARM // Decryption Failure: {decrypt_err}")
...
components.html(BOOT_ANIMATION_HTML, height=0)